/**
 * Structured JSON logger for the PR Gatekeeper.
 *
 * Writes to **stderr** only — stdout is owned by the stdio MCP transport.
 */
...
function emit(level: LogLevel, msg: string, extra: Record<string, unknown>): void {
  ...
  _writer(
    JSON.stringify({ level, ts: new Date().toISOString(), service: "pr-gatekeeper", msg, ...extra }),
  );
}
