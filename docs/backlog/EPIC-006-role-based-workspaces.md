...
# TASK-119: Quick-job lane (capture → do → bill, no estimate)
...
Problem:
From a workflow review with the owner: for quick jobs ("come assemble a bed"),
the app is abandoned — "sometimes just in my head or write it down because
trying to use the app is difficult" — and then a manual invoice is made,
re-entering everything. The job is lost at the source: hours, client history,
repeat-work signal. The whole app is shaped around the big-job spine
(estimate → job → visits → invoice); a quick, no-estimate, hourly job has no
fast lane, so it never gets captured. This is the #1 end-user accessibility
failure. Design source: `docs/working/2026-09-05-quick-job-lane-spec.md`.

Business Value:
The high-frequency small jobs actually land in the system — scheduled, timed,
and billed — instead of on paper. Hours flow to the invoice automatically; the
client relationship and history accrue.

Scope:
- **Capture in one step:** a "Quick job" action = client (pick or type new) +
  title + schedule slot, creating the Job+Visit behind the scenes. Extends the
  existing "Quick Project" path (`apps/web/app/app/jobs/new`) to also place the
  visit on the schedule.
- **Time auto-captured:** reuse the existing clock / `activity_entries` + arrival
  push; one tap start/stop on the visit.
- **Bill in one tap:** invoice from the job with hours pre-filled at
  `labor_billing_cents_per_hour` via the existing tracked-time→labor bridge
...
- The 4-labor-rate reconciliation beyond using the one bill rate (tracked in
  pricing work / PI-002/004).
- Tech multi-user targeting (owner/solo first).

Acceptance Criteria:
- [ ] A quick job is created + scheduled in one flow, no estimate, no Project
      scaffolding step.
- [ ] On-site time is captured and appears on the invoice without manual entry.
- [ ] Invoice defaults to hourly at the bill rate; can switch to a price-book
      rate or flat fee in one action.
- [ ] The job, its client, and its hours are queryable afterward (not lost).

Notes:
Priority item from the 2026-09-05 owner workflow review. Sibling to TASK-075
(fewer taps). Detailed design + flow: the spec file above.

# TASK-121: Big-job execution — synced task check-off + unified job spend
...
Problem:
On multi-day jobs the owner wants to check off tasks (paint room, demo bath,
replace toilet) individually as they go, from **either** My Day during the visit
**or** the Job page — synced. Separately, job materials live in two places (the
planned buy list and ad-hoc expense receipts), so there is no single view of what
was planned vs actually spent on a job.

Business Value:
Clear per-task progress across days, and one honest "job materials & spend"
picture (planned buy list + ad-hoc receipts) feeding job cost.

Scope:
- Work Orders (the task model) get a check-off state surfaced and editable on
  both My Day (per visit) and the Job page, kept in sync.
- A single "job materials & spend" view merging the buy list and receipts already
  attached to the job (buy list + `expenses` receipts), rolling into job cost.

Out of Scope:
- Changing the Job→Visit→Work Order data model.
- Per-task time allocation (job/visit-level time is enough for now).

Acceptance Criteria:
- [ ] Checking a task off in My Day reflects on the Job page and vice-versa.
- [ ] One job view shows planned buy-list items and ad-hoc receipts together with
      a spend total.

Notes:
From the 2026-09-05 owner workflow review (big-job lane, "check off each task as I
go"; materials "mix of buy list + ad-hoc receipts").
