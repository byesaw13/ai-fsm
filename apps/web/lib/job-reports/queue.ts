...
export const REPORT_QUEUE_WHERE = `
  ...
  AND EXISTS (
    ...
    WHERE v.job_id = j.id AND v.account_id = j.account_id AND vm.category = ANY($2::text[]))
  AND NOT EXISTS (
    SELECT 1 FROM portal_job_updates r
    WHERE r.job_id = j.id AND r.account_id = j.account_id
      AND r.status IN ('published', 'withdrawn', 'skipped'))`;
...
export async function loadReportQueue(db: PoolClient, accountId: string): Promise<ReportQueueRow[]> {
  const { rows } = await db.query<ReportQueueRow>(
    `SELECT j.id::text AS job_id, j.title, c.name AS client_name, p.address,
            COALESCE((SELECT max(v.completed_at) FROM visits v
                      WHERE v.job_id = j.id AND v.account_id = j.account_id), j.updated_at)::text AS finished_at,
            (SELECT count(*)::int FROM visit_media vm
             JOIN visits v ON v.id = vm.visit_id AND v.account_id = vm.account_id
             WHERE v.job_id = j.id AND v.account_id = j.account_id AND vm.category = ANY($2::text[])) AS photo_count,
            EXISTS (SELECT 1 FROM portal_job_updates r
                    WHERE r.job_id = j.id AND r.account_id = j.account_id AND r.status = 'draft') AS has_draft,
            EXISTS (SELECT 1 FROM invoices i
                    WHERE i.job_id = j.id AND i.account_id = j.account_id
                      AND i.status NOT IN ('draft', 'void')) AS has_billed_invoice
     FROM jobs j
     ...
     -- No cap: the Needs Attention count uses the same predicate, uncapped.
