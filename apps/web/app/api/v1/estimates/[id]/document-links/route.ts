...
export const PATCH = withRole(["owner", "admin"], async (request, session) => {
  ...
  try {
    const result = await withEstimateContext(session, async (client) => {
      ...
      const targetClientId = input.client_id ?? est.client_id;
      if (input.client_id) {
      ...
      if (input.location_mode === "job" || input.job_id) {
        const jobId = input.job_id ?? est.job_id;
        if (!jobId) {
      ...
      const patchInput = {
        ...
        job_id: input.location_mode === "job" ? (input.job_id ?? est.job_id) : input.job_id,
      };

      const next = resolveDocumentLinkPatch(patchInput, est, jobPropertyId);
      ...
      if (
        next.client_id === est.client_id
        && next.job_id === est.job_id
        && next.property_id === est.property_id
      ) {
    ...
    logger.error("PATCH /api/v1/estimates/[id]/document-links error", error, { traceId: session.traceId });
    return NextResponse.json(
