...
interface Props {
  ...
  onClose: () => void;
  /** After book, go here instead of the job page. Field launches use My Day. */
  successHref?: string;
...
export function QuickBookModal({
  ...
  // Schedule
  const [date, setDate] = useState(nowSlot?.date ?? initialDate);
  const [startTime, setStartTime] = useState(nowSlot?.time ?? "08:00");
  const [duration, setDuration] = useState(startNow ? 60 : 120);
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
          ...
          <div>
            ...
            <button
              type="button"
              onClick={() => {
                const slot = nextHalfHourLocal();
                setDate(slot.date);
                setStartTime(slot.time);
                setDuration(60);
              }}
              style={{ ...btnStyle, marginTop: 8, background: "var(--bg-muted, #f4f4f5)", color: "var(--fg)", padding: "6px 12px" }}
            >
              Now
            </button>
          </div>
