# Design: Stabilize Dovetails FSM — Works First, Clean on Contact

Generated by office-hours on 2026-07-03  
Status: DRAFT  
Mode: Intrapreneurship (Startup diagnostic posture)
...
- **Active operational dependency:** Recent commits (#447–#454) ship My Day, work-order-centric field model, activity timeline, location/day review — the business runs on this app today.
- **Live pain:** Branch `fix/my-day-start-mileage-clock` exists because start-day mileage and clocking are blocked — field ops cannot wait for a rebuild.
...
Public booking / intake / SMS
  ...
  → project approval
  → work order → visit (My Day field execution)
...
**Narrowest wedge (this week):** Technician can start the day (odometer + clock), execute assigned work orders/visits, and close the day without modal dead-ends or blocked flows.

**Not the wedge:** Migration squash, `packages/db` extraction, portal expansion, location intelligence EPICs, membership subsystem.
...
5. **Scope freeze** — no new EPICs until revenue spine + field ops are boringly reliable.
...
- Effort S | Risk Low
- Rejected as sole path: stops the bleeding but doesn't address "clean" (priority C).
...
### Phase 0 — Works (immediate, days)

**Goal:** Field ops boringly reliable.

| Task | Source |
|------|--------|
| Land `fix/my-day-start-mileage-clock` (modal stacking, start-day mileage + clock) | Active branch |
| Run `pnpm gate:fast` before merge; `pnpm gate` if field paths touched | Quality gates |
| Smoke-verify: start day → view work order → clock activity → end day review | Manual + existing E2E (`my-day-mobile`, `day-review`) |

**Definition of done:** Owner/tech can run a full field day without workaround or blocked UI.

### Phase 1 — Stop the Bleeding (ongoing rules)

Three standing rules — no sprint required:

**Rule 1: Scope freeze**
- No new tables, routes, or EPICs (007 location, 008 PI stub, portal, membership) unless they fix a P0 field or revenue-spine bug.
...
### Phase 2 — Clean on Contact (months, opportunistic)
...
### Works (A) — measurable within 2 weeks
...
### Clean (B) — measurable within 90 days
...
| Blocker | Status |
|---------|--------|
| `fix/my-day-start-mileage-clock` merge | In progress |
| Canonical docs alignment | Exists (`docs/canonical/`) — use as scope gate |
| REBUILD_AUDIT_REPORT.md | Inventory reference only — **Section 23 blueprint is out of scope** |
...
Finish and merge `fix/my-day-start-mileage-clock`, then run a real field day on production (or staging): start day with odometer, clock in, open a work order, complete one visit action, open day review. If anything blocks, file it as P0 — nothing else gets worked until that flow is clean.
...
- You brought a rebuild audit but immediately rejected rebuild — that's conviction, not compliance. The audit stays useful as inventory and "what not to add," not as a execution plan.
- You said "clean app that works" and chose **C (both, A first)** — that maps exactly to Approach B; you don't actually have a strategy conflict, you had a document mismatch.
- Active work on My Day proves field ops are P0, which invalidates the audit's Phase 1 cutover assumption. You were right to push back.

---

## Founder Signals (session)

| Signal | Evidence |
|--------|----------|
| Real problem | "I just want a clean app that works" |
| Specific users | Dovetails owner/admin/tech roles from audit + live usage |
| Pushed back on premises | Rejected full rebuild explicitly |
| Domain expertise | Knows My Day / mileage / clock pain from active branch |
| Agency | Shipping on current codebase, not planning forever |
| Taste | Wants coherence — "clean" — not just bug fixes |